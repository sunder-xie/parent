...
<script type="text/javascript">
	function optDelete(name,isDisplay,pageNo){
		...
		$("#jvForm").attr({"action":"/brand/deletes.do?name="+name+"&isDisplay="+isDisplay+"&pageNo="+pageNo,"method":"post"});
		$("#jvForm").submit();
	}
	...
	<tbody class="pn-ltbody">
		...
		<c:forEach items="${pagination.list}" var="brand">
			<tr bgcolor="#ffffff" onmouseout="this.bgColor='#ffffff'" onmouseover="this.bgColor='#eeeeee'">
				...
				<td align="center">
				<a class="pn-opt" href="/brand/toEdit.do?id=${brand.id }">修改</a> | <a class="pn-opt" onclick="if(!confirm('您确定删除吗？')) {return false;}" href="/brand/deletes.do?ids=${brand.id }">删除</a>
				</td>
			</tr>
		</c:forEach>
	</tbody>
